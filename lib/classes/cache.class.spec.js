...
describe('Simple Stash', () => {
    ...
    it('should return an empty array if no results are found', () => {
        ...
        const result = JSON.stringify(aCache.getAll('users:value:does not exist'));
    ...
    it('should return null or empty if there is no search input', () => {
        ...
        expect(JSON.stringify(aCache.getAll(null))).to.equal(JSON.stringify([]));
    });
    ...
    it('should allow clearing the cache', () => {
        ...
        expect(JSON.stringify(aCache.get('users:id:some-id')))
            .to.equal(JSON.stringify(testData[0]));
